...
def loginView(request):
    if request.method == 'POST':
        ...
        if user is not None and user.is_active:
            ...
            messages.info(request, 'Invalid username or password so do check it and try again.')
            return redirect('login')
    else:
...
def patient_home(request):
    ...
    if not user_profile:
        context = {'profile_status': 'Please create a profile to continue.', 'doctor': doctor, 'ment':appointment, patient:'patient', 'drug':medical3}
